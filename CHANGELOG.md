...
- **Added** `--concurrency` flag to limit the number of tasks running at the same time, supporting both absolute values (e.g. `--concurrency 5`) and percentages of CPU cores (e.g. `--concurrency 50%`) ([#288](https://github.com/voidzero-dev/vite-task/pull/288), [#309](https://github.com/voidzero-dev/vite-task/pull/309))
- **Added** `--parallel` flag to ignore task dependencies and run all tasks at once with unlimited concurrency (unless `--concurrency` is also specified) ([#309](https://github.com/voidzero-dev/vite-task/pull/309))
