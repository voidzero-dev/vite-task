//! Glob matching for filesystem **paths** with gitignore semantics.
//!
//! Backed by `wax`. `!`-prefixed patterns negate. With no negation present,
//! first-match-wins; with any negation, last-match-wins (gitignore).
...
/// Error compiling a path glob pattern.
#[derive(Debug, thiserror::Error)]
#[error(transparent)]
pub struct PathGlobError(#[from] wax::BuildError);

/// Matches filesystem paths against an ordered set of glob patterns.
///
...
impl<'a> PathGlobSet<'a> {
    ...
    pub fn new<I, S>(match_patterns: I) -> Result<Self, PathGlobError>
    where
...
mod tests {
    ...
    #[test]
    fn test_match_ignores_node_modules() -> Result<(), PathGlobError> {
        let patterns = vec![
            // ignore all paths
            "**/*",
    ...
    #[test]
    fn test_match_ignores_with_file_patterns() -> Result<(), PathGlobError> {
    ...
    #[test]
    fn test_match_ignores_directory_patterns() -> Result<(), PathGlobError> {
    ...
    #[test]
    fn test_match_ignores_complex_patterns() -> Result<(), PathGlobError> {
        let patterns = vec![
            "**/*.test.js",
            "**/*.spec.ts",
            "**/test/**",
    ...
    #[test]
    fn test_match_ignores_empty_patterns() -> Result<(), PathGlobError> {
    ...
    #[test]
    fn test_match_ignores_with_wildcards() -> Result<(), PathGlobError> {
    ...
    #[test]
    fn test_match_ignores_dotfiles() -> Result<(), PathGlobError> {
    ...
    #[test]
    fn test_match_ignores_root_patterns() -> Result<(), PathGlobError> {
    ...
    #[test]
    fn test_match_ignores_directory_only_patterns() -> Result<(), PathGlobError> {
        let patterns = vec![
    ...
    #[test]
    fn test_match_ignores_mixed_patterns() -> Result<(), PathGlobError> {
        let patterns = vec![
    ...
    #[test]
    fn test_generic_api_with_different_types() -> Result<(), PathGlobError> {
        use vt_str::Str;
        ...
        // Test with Vec<Str>
        let patterns_vt_str = vec![Str::from("*.rs"), Str::from("!main.rs")];
        let ignores_vt_str = PathGlobSet::new(&patterns_vt_str)?;
        assert!(ignores_vt_str.is_match("lib.rs"));
        assert!(!ignores_vt_str.is_match("main.rs"));
    ...
    #[test]
    fn test_match_ignores_last_matching_pattern() -> Result<(), PathGlobError> {
