#[expect(
    ...
    reason = "StripPrefixError carries a raw &Path that may not be valid UTF-8, so it can't use vt_path types"
)]
use std::path::Path;
use std::{io, sync::Arc};
...
pub enum Error {
    ...
    #[expect(
        clippy::disallowed_types,
        reason = "stripped path may not be valid UTF-8, so it can't use vt_path types"
    )]
    ...
    #[error(transparent)]
    Glob(#[from] vt_glob::path::PathGlobError),
}
