...
The macOS backend uses named POSIX shared memory. Another process opens the
same bytes using the POSIX name. Pages are allocated as they are accessed, and
dropping the owner removes the name. No broker is required.
...
`shared_memory` calls the same POSIX APIs. Fspy only needs to create, open,
map, and unlink shared memory, so the backend calls those APIs directly.
