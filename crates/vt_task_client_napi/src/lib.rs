...
use vt_task_client::{Client, GetEnvsQuery};
...
impl RunnerClient {
    ...
    pub fn ignore_input(&self, path: String) -> Result<()> {
        self.client
            ...
            .map_err(|err| err_string(vt_str::format!("{err}")))
    }
        ...
        let value = value
            .map(|value| {
                value.to_str().map(str::to_owned).ok_or_else(|| {
                    err_string(vt_str::format!("env value for {name} is not valid UTF-8"))
                })
            })
            .transpose()?;
        Ok(value.into())
...
pub fn load() -> Result<RunnerClient> {
    ...
    let client = Client::from_envs(std::env::vars_os())
        .map_err(|err| {
            err_string(vt_str::format!("vp run client: failed to connect to runner IPC: {err}"))
        })?
        .ok_or_else(|| {
            err_static(
...
fn err_string(msg: vt_str::Str) -> Error {
