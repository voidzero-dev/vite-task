...
pub struct InvalidGlob {
    pub pattern: Box<str>,
    pub source: vt_glob::env::EnvGlobError,
}
        ...
        tokio::select! {
            ...
            accept_result = bound.accept() => {
                match accept_result {
                    ...
                    Err(err) => {
                        tracing::warn!(?err, "vt_task_server: accept failed");
                    }
                }
            }
...
async fn handle_client<H: Handler>(mut stream: Stream, handler: &RefCell<H>) -> Result<(), Error> {
    ...
    loop {
        ...
        // this driver drains. See `Request` in `vt_task_ipc_shared` for
        // the rationale.
        match request {
