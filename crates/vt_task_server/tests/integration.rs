...
use vt_task_ipc_shared::{GetEnvResponse, Request};
use vt_task_server::{EnvQuery, Error, Recorder, Reports, ServerHandle, serve};
...
fn relative_input_joined_with_cwd() {
    let cwd = vt_path::current_dir().expect("cwd");
