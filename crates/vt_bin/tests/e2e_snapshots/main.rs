        ...
        let command = match self {
            ...
            Self::Detailed(config) => {
                ...
                for (k, v) in &config.envs {
                    parts.push_str(vt_str::format!("{k}={v} ").as_str());
                }
...
fn run_case(
    ...
    let e2e_stage_path = tmpdir.join(vt_str::format!("{fixture_name}_case_{case_index}"));
    ...
    e2e_outputs.push_str(vt_str::format!("# {}\n", e2e.name).as_str());
    ...
    }
    {
        for step in &e2e.steps {
            ...
            let mut cmd = if matches!(program, "vt" | "vtt") {
                let exe_env = vt_str::format!("CARGO_BIN_EXE_{program}");
                let resolved =
                ...
                for interaction in interactions {
                    match interaction {
                        ...
                        Interaction::WriteKey(write_key) => {
                            ...
                            output_for_thread.lock().unwrap().push_str(
                                vt_str::format!("**← write-key:** `{key_name}`\n\n").as_str(),
                            );
            ...
            match &termination_state {
                ...
                TerminationState::Exited(exit_code) => {
                    if *exit_code != 0 {
                        e2e_outputs
                            .push_str(vt_str::format!("**Exit code:** {exit_code}\n\n").as_str());
                    }
                }
            }
    ...
    snapshots.check_snapshot(vt_str::format!("{}.md", e2e.name).as_str(), &e2e_outputs)?;
    Ok(())
}
