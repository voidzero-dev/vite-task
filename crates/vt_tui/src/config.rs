#[expect(
    ...
    reason = "vt_tui is a standalone TUI app, not using vt_path/vt_str"
)]
...
#[expect(
    ...
    reason = "vt_tui is a standalone TUI app, not using vt_str"
)]
pub static PROJECT_NAME: LazyLock<String> =
    LazyLock::new(|| env!("CARGO_CRATE_NAME").to_uppercase());
// pub static DATA_FOLDER: LazyLock<Option<PathBuf>> =
// LazyLock::new(|| env::var(format!("{}_DATA", PROJECT_NAME.clone())).ok().map(PathBuf::from));
// pub static CONFIG_FOLDER: LazyLock<Option<PathBuf>> =
// LazyLock::new(|| env::var(format!("{}_CONFIG", PROJECT_NAME.clone())).ok().map(PathBuf::from));
...
#[expect(clippy::disallowed_types, reason = "vt_tui is a standalone TUI app, not using vt_path")]
...
fn project_directory() -> Option<ProjectDirs> {
    ProjectDirs::from("dev", "voidzero", env!("CARGO_PKG_NAME"))
}
