...
pub trait Handler {
    ...
    /// glob, recording the match-set for the post-run fingerprint.
    ///
    /// # Errors
    ///
...
/// — `vite_glob::env::EnvGlobError` wraps `globset::Error`, which is large.
#[derive(Debug)]
    ...
    fn get_envs(
        ...
        let matches: FxHashMap<Arc<OsStr>, Arc<OsStr>> = self
            ...
            .filter_map(|(name, value)| {
                ...
                // glob, so they're silently dropped. Values can still be
                // non-UTF-8 — that conversion happens at the wire-encoding /
                // fingerprinting boundary, not here.
