...
//! `find_ps1_sibling`, `is_stdin_terminal`) live in the `vt_powershell`
...
pub fn rewrite_cmd_shim_with_args(
    ...
    if let Some(host) = vt_powershell::powershell_host()
        && let Some(rewritten) = rewrite_with_host(
            ...
            workspace_root,
            host,
            vt_powershell::is_stdin_terminal(),
        )
    {
        return rewritten;
    }
...
fn rewrite_with_host(
    ...
    let ps1 = vt_powershell::find_ps1_sibling(resolved)?;
    ...
    let new_args: Arc<[Str]> = vt_powershell::POWERSHELL_PREFIX
        .iter()
        .copied()
        .map(Str::from)
