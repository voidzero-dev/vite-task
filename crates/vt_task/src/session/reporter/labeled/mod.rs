...
impl LeafExecutionReporter for LabeledLeafReporter {
    fn start(&mut self, cache_status: CacheStatus) -> StdioConfig {
        ...
        let labeled_line = vt_str::format!("{label} {line}");
        let mut writer = self.writer.borrow_mut();
        let _ = writer.write_all(labeled_line.as_bytes());
        let _ = writer.flush();

        let prefix = vt_str::format!("{label} ");
...
#[cfg(test)]
mod tests {
    use vt_task_plan::ExecutionItemKind;
    ...
    fn leaf_kind(item: &vt_task_plan::ExecutionItem) -> &LeafExecutionKind {
        match &item.kind {
