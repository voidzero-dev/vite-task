...
use vt_str::Str;
use vt_task_plan::{ExecutionItemDisplay, LeafExecutionKind};
...
fn format_cwd_relative(display: &ExecutionItemDisplay, workspace_path: &AbsolutePath) -> Str {
    ...
    if cwd_relative.is_empty() { Str::default() } else { vt_str::format!("~/{cwd_relative}") }
}
...
fn format_task_label(display: &ExecutionItemDisplay) -> Str {
    vt_str::format!(
        "{}",
        vt_str::format!("[{}]", display.task_display).style(Style::new().bright_black())
    )
}
...
fn format_command_display(display: &ExecutionItemDisplay, workspace_path: &AbsolutePath) -> Str {
    ...
    vt_str::format!("{cwd_str}$ {}", display.command)
}
        ...
        |inline_status| {
            let styled_status = inline_status.split_once(' ').map_or_else(
                ...
                |(symbol, text)| {
                    ...
                    vt_str::format!("{} {}", symbol.style(symbol_style), text.style(text_style))
                        .to_string()
                },
            );

            vt_str::format!("{} {styled_status}\n", command_str.style(COMMAND_STYLE))
        },
    )
}
