...
use vt_workspace::{WorkspaceRoot, find_workspace_root, package_graph::PackageQuery};
...
impl TaskGraphLoader for LazyTaskGraph<'_> {
    async fn load_task_graph(
        &mut self,
    ) -> Result<&vt_task_graph::IndexedTaskGraph, TaskGraphLoadError> {
...
#[async_trait::async_trait(?Send)]
impl vt_task_plan::PlanRequestParser for PlanRequestParser<'_> {
...
impl<'a> Session<'a> {
                ...
                let graph = if let Some(ref task_specifier) = run_command.task_specifier {
                    ...
                    if plan_result.graph.graph.node_count() == 0 {
                        ...
                        if is_cwd_only && !has_execution_flags {
                            ...
                            return Err(vt_task_plan::Error::NoTasksMatched(
                    ...
                    if normalized_run_command != bare {
                        return Err(vt_task_plan::Error::MissingTaskSpecifier.into());
                    }
                    let qpr = self.handle_no_task(is_interactive, &run_command).await?;
                    self.plan_from_query(qpr).await?
                };
    ...
    async fn handle_no_task(
        ...
        let select_items: Vec<SelectItem> = entries
            ...
            .map(|entry| {
                ...
                let label = if is_current {
                    ...
                    vt_str::format!("{}", entry.task_display)
                };
                ...
                let group = if is_current {
                    ...
                    Some(if pkg_name.is_empty() {
                        ...
                        vt_str::format!("{pkg_name} ({display_path})")
                    })
                };
        ...
        let header = not_found_name.map(|name| {
            if is_interactive {
                vt_str::format!("Task \"{name}\" not found.")
            } else {
                let labels: Vec<&str> =
                    select_items.iter().map(|item| item.label.as_str()).collect();
                let has_suggestions = !vt_select::fuzzy_match(name, &labels).is_empty();
                if has_suggestions {
                    vt_str::format!("Task \"{name}\" not found. Did you mean:")
                } else {
                    vt_str::format!("Task \"{name}\" not found.")
                }
            }
        ...
        let mode =
            selected_index.as_mut().map_or(vt_select::Mode::NonInteractive, |selected_index| {
                vt_select::Mode::Interactive { selected_index }
            });
        ...
        let select_result = vt_select::select_list(&mut stdout, &params, mode, |state| {
            let milestone_name =
                vt_str::format!("task-select:{}:{}", state.query, state.selected_index);
            pty_terminal_test_client::mark_milestone(&milestone_name);
        })?;

        if matches!(select_result, vt_select::SelectResult::Cancelled) {
    ...
    pub async fn execute_synthetic(
        ...
        let spawn_execution = vt_task_plan::plan_synthetic(
            &self.workspace_path,
            &self.cwd,
    ...
    pub async fn plan_from_cli_run(
        ...
        cwd: Arc<AbsolutePath>,
        command: RunCommand,
    ) -> Result<ExecutionGraph, vt_task_plan::Error> {
        let (plan_result, _) =
    ...
    async fn plan_from_cli_run_resolved(
    ...
    ) -> Result<(vt_task_plan::PlanResult, bool), vt_task_plan::Error> {
        let (query_plan_request, is_cwd_only) = match command.into_query_plan_request(&cwd) {
            Ok(result) => result,
            Err(crate::cli::CLITaskQueryError::MissingTaskSpecifier) => {
                return Err(vt_task_plan::Error::MissingTaskSpecifier);
            }
            Err(error) => {
                return Err(vt_task_plan::Error::ParsePlanRequest {
                    error: error.into(),
        ...
        let plan_result = vt_task_plan::plan_query(
            query_plan_request,
            &self.workspace_path,
            &cwd,
            &self.envs,
