...
use vt_str::Str;
use vt_task_plan::cache_metadata::{CacheMetadata, EnvValueHash};
use vt_task_server::Reports;
...
fn collect_tracked_env_queries(reports: &Reports) -> anyhow::Result<TrackedEnvQueryValues> {
    ...
    for (query, record) in &reports.tracked_get_envs {
        ...
        let query = match query {
            vt_task_server::EnvQuery::Glob(pattern) => {
                TrackedEnvQuery::Glob(Str::from(pattern.as_ref()))
            }
            vt_task_server::EnvQuery::Prefix(prefix) => {
