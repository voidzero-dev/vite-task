//! The `vt_task_client_napi` cdylib is embedded into the `vp` binary and
...
/// Path to the materialized `vt_task_client_napi` `.node` addon.
///
...
pub fn napi_client_path() -> &'static AbsolutePath {
    static PATH: LazyLock<AbsolutePathBuf> = LazyLock::new(|| {
        let dir = env::temp_dir().join("vt_task_client_napi");
        let _ = fs::create_dir(&dir);
        let path = artifact!("vt_task_client_napi")
            .materialize()
            .suffix(".node")
            .at(&dir)
            .expect("materialize vt_task_client_napi");
