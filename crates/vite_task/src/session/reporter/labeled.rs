    ...
    fn new_leaf_execution(&mut self, path: &LeafExecutionPath) -> Box<dyn LeafExecutionReporter> {
        ...
        let stdio_suggestion = match &item.kind {
            ExecutionItemKind::Leaf(LeafExecutionKind::Spawn(_))
                if path.all_containing_graphs_single_node() =>
            {
...
mod tests {
    ...
    /// Build a [`LeafExecutionPath`] for the given graph and path indices.
    ///
    /// All items except the last are treated as intermediate `Expanded` steps;
    /// the last becomes the leaf. Uses [`ExecutionPathPrefix`] to construct the
    /// path with correct precomputed metadata.
    fn leaf_path(graph: &ExecutionGraph, path_items: &[(usize, usize)]) -> LeafExecutionPath {
        let (&(leaf_node, leaf_item), ancestors) = path_items
            .split_last()
            .expect("leaf_path requires at least one path item");

        let mut prefix = ExecutionPathPrefix::new(graph);
        let mut current_graph = graph;
        for &(node_ix, item_ix) in ancestors {
            let item = &current_graph[ExecutionNodeIndex::new(node_ix)].items[item_ix];
            let ExecutionItemKind::Expanded(nested) = &item.kind else {
                panic!("leaf_path: intermediate item ({node_ix}, {item_ix}) is not Expanded");
            };
            prefix = prefix.extended(ExecutionNodeIndex::new(node_ix), item_ix, nested);
            current_graph = nested;
        }
        prefix.to_leaf_path(ExecutionNodeIndex::new(leaf_node), leaf_item)
    }
    ...
    async fn labeled_reporter_single_spawn_in_single_node_chain_suggests_inherited() {
        let graph = Arc::new(ExecutionGraph::from_node_list([spawn_task("build")]));
        let path = leaf_path(&graph, &[(0, 0)]);
    ...
    async fn labeled_reporter_multi_node_root_graph_suggests_piped() {
        let graph = Arc::new(ExecutionGraph::from_node_list([
            spawn_task("build"),
            spawn_task("test"),
        ]));
        let path = leaf_path(&graph, &[(0, 0)]);
    ...
    async fn labeled_reporter_nested_single_node_chain_suggests_inherited() {
        ...
        let graph = Arc::new(ExecutionGraph::from_node_list([expanded_task(
            "expand", nested,
        )]));
        let path = leaf_path(&graph, &[(0, 0), (0, 0)]);
    ...
    async fn labeled_reporter_keeps_inherited_for_single_node_chains_with_multi_node_and_sibling() {
        ...
        let graph = Arc::new(graph_with_mixed_and_siblings());
        let path_top_level = leaf_path(&graph, &[(0, 0)]);
        let path_nested_single = leaf_path(&graph, &[(0, 1), (0, 0)]);
        let path_nested_multi = leaf_path(&graph, &[(0, 2), (0, 0)]);

        assert_eq!(
            suggestion_for_path(&graph, &path_top_level).await,
            StdioSuggestion::Inherited
        );
        assert_eq!(
            suggestion_for_path(&graph, &path_nested_single).await,
            StdioSuggestion::Inherited
        );
        assert_eq!(
            suggestion_for_path(&graph, &path_nested_multi).await,
    ...
    async fn labeled_reporter_in_process_leaf_suggests_piped() {
        let graph = Arc::new(ExecutionGraph::from_node_list([in_process_task("echo")]));
        let path = leaf_path(&graph, &[(0, 0)]);
