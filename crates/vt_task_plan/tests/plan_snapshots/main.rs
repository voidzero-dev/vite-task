...
use vt_str::Str;
use vt_task::{Command, Session};
use vt_task_graph::display::TaskDisplay;
use vt_task_plan::{ExecutionGraph, ExecutionItemKind};
...
impl CompactPlan {
    ...
    fn task_key(task_display: &TaskDisplay, workspace_root: &AbsolutePath) -> Str {
        ...
        vt_str::format!("{}#{}", relative, task_display.task_name)
    }
}
...
fn run_case_inner(
    ...
    runtime.block_on(async {
        ...
        let mut owned_config = vt_task_bin::OwnedSessionConfig::default();
        ...
        let task_graph = match task_graph_result {
            ...
            Err(err) => {
                let err_formatted = vt_str::format!("{err:#}");
        ...
        for plan in cases_file.plan_cases {
            ...
            let mut case_owned_config = vt_task_bin::OwnedSessionConfig::default();
            ...
            let plan = match plan_result {
                ...
                Err(err) => {
                    ...
                    let err_formatted = vt_str::format!("{anyhow_err:#}");
                    let err_str =
            ...
            let comment = vt_str::format!("{args_display}");
            if compact {
