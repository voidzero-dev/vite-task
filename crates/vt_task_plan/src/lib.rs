...
use vt_str::Str;
use vt_task_graph::{TaskGraphLoadError, display::TaskDisplay};

/// A resolved spawn execution.
///
/// Unlike tasks in `vt_task_graph`, this struct contains all information needed for execution,
...
impl vt_graph_ser::GetKey for TaskExecution {
    type Key<'a> = (&'a AbsolutePath, &'a str);

    #[expect(
        clippy::disallowed_types,
        reason = "vt_graph_ser::GetKey uses String in its trait definition"
    )]
...
pub trait PlanRequestParser: Debug {
    ...
    ///
    /// `vt_task_plan` doesn't have the knowledge of how cli args should be parsed. It relies on this callback.
    ///
...
const fn resolve_cache_with_override(
    graph_cache: vt_task_graph::config::ResolvedGlobalCacheConfig,
    cache_override: CacheOverride,
) -> vt_task_graph::config::ResolvedGlobalCacheConfig {
    match cache_override {
        CacheOverride::ForceEnabled => {
            vt_task_graph::config::ResolvedGlobalCacheConfig { scripts: true, tasks: true }
        }
        CacheOverride::ForceDisabled => {
            vt_task_graph::config::ResolvedGlobalCacheConfig { scripts: false, tasks: false }
        }
