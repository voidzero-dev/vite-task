...
use vt_path::{AbsolutePath, relative::InvalidPathDataError};
use vt_str::Str;
...
pub enum Error {
    ...
    #[error(transparent)]
    PackageQueryResolve(#[from] vt_task_graph::query::PackageQueryResolveError),

    #[error("Failed to load task graph")]
    TaskGraphLoad(
        #[source]
        #[from]
        vt_task_graph::TaskGraphLoadError,
    ),
    ...
    ResolveTaskConfig(
        #[source]
        #[from]
        vt_task_graph::config::ResolveTaskConfigError,
    ),
    ...
    #[error(
        ...
        sources.iter().map(vt_str::Str::as_str).collect::<Vec<_>>().join(", ")
    )]
